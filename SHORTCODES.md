...
**actual plugin** (`website/wp-mu-plugin/vc-clients-embed.php`, v2.4.0) — not aspirational.
...
`[brg_nav]` content is a WordPress menu; its **layout is set by shortcode attributes** (v2.4.0):
...
| `sticky` | `pin` | `pin` stays; `sticky="hide"` = hide-on-scroll-down (nav is `fixed`, so it always sticks) |
| `bg` | `solid` | `solid` (white bar, reserves space) · `none` (transparent, floats over the hero) · `frost` (dark frosted glass + blur) |
| `bgcolor` | `#fff` | background color for solid/frost, any CSS color (e.g. `bgcolor="#131210"`) |
...
- ~~Nav layout modes~~ — **DONE (v2.4.0):** `layout`/`left`/`right`/`sticky` + More drawer.
